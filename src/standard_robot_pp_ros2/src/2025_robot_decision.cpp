...
namespace standard_robot_pp_ros2
{
    ...
    // 参数声明部分修改为完整命名空间路径
        ...
        const int supply_safe_point_index = declare_ns_param(
            "point_roles.supply_safe_point_index", 0);
        const auto patrol_indices_raw = declare_ns_param(
            "point_roles.patrol_indices", std::vector<int64_t>{1, 2, 3});
        const auto low_hp_candidate_indices_raw = declare_ns_param(
            "point_roles.low_hp_candidate_indices", std::vector<int64_t>{1, 2, 3});
        const int critical_time_target_index = declare_ns_param(
            "point_roles.critical_time_target_index", 1);
        ...
        supply_safe_point_index_ = validate_single_index(
            supply_safe_point_index, "point_roles.supply_safe_point_index");
        patrol_indices_ = validate_index_list(
            patrol_indices_raw, "point_roles.patrol_indices");
        low_hp_candidate_indices_ = validate_index_list(
            low_hp_candidate_indices_raw, "point_roles.low_hp_candidate_indices");
        critical_time_target_index_ = validate_single_index(
            critical_time_target_index, "point_roles.critical_time_target_index");
        patrol_cursor_ = 0;
        patrol_direction_ = 1;
    ...
    params_.config.decision_period_ms =
        std::clamp(params_.config.decision_period_ms, 20, 1000);
    ...
    RCLCPP_INFO(
        get_logger(),
        "补给安全点索引: %zu 坐标: (%.2f, %.2f, %.2f)",
        supply_safe_point_index_,
        rmul_goal_point_[supply_safe_point_index_].x,
        rmul_goal_point_[supply_safe_point_index_].y,
        rmul_goal_point_[supply_safe_point_index_].z);
    RCLCPP_INFO(
        get_logger(),
        "巡逻点列表: %s",
        formatIndexList(patrol_indices_).c_str());
    RCLCPP_INFO(
        get_logger(),
        "低血量候选列表: %s",
        formatIndexList(low_hp_candidate_indices_).c_str());
    RCLCPP_INFO(
        get_logger(),
        "时间严重紧张目标索引: %zu 坐标: (%.2f, %.2f, %.2f)",
    ...
    // 订阅比赛状态、RFID状态和机器人状态消息
        ...
        timer_ = create_wall_timer(
            std::chrono::milliseconds(params_.config.decision_period_ms),
            [this]() {
    ...
    int RobotDecisionNode::getGameTimeState()
    {
        ...
        else if (game_status_msg_.stage_remain_time > 0 &&game_status_msg_.stage_remain_time <= params_.time.critical) {
            return 4; // 时间严重紧张
        }
        else {
    ...
    void RobotDecisionNode::robotDecision()
    {
        ...
        //         break;
        //     case 4:
        //         if (critical_time_target_index_ < rmul_goal_point_.size()) {
        //             sendNavigationGoal(
        //                 rmul_goal_point_[critical_time_target_index_].x,
        //                 rmul_goal_point_[critical_time_target_index_].y,
        //                 rmul_goal_point_[critical_time_target_index_].z);
        ...
        //         break;
        //     default:
        //         RCLCPP_DEBUG(get_logger(), "当前时间状态无动作: %d", current_game_state);
        //         break;
        // }
    ...
    {
        RCLCPP_WARN_THROTTLE(
            get_logger(), *this->get_clock(), 2000, "血量危急，回到安全点");
